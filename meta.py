...
config = {'manual': False,'info_level': 0, 'restricted_extensions': [], 'ignore_files': ["meta.xml", "meta-generated.xml"], 'author': "Default", 'cache': False, 'override': True, 'force_override': True,'generate_exported': True}
...
if len(sys.argv) > 1 and os.path.isdir(sys.argv[1]):
    ...
    with open("meta-generated.xml", "w+") as meta_file:
        ...
        description = "Generated by MTA-Resource-Tool"
        ...
        meta_file.write("\t" + "<info name=\"" + name +"\" description=\"" + description + "\" author=\"" + author + "\"/>" + "\n")
        
        if resource:
            if 'oop' in resource and resource['oop']:
                meta_file.write("\t" + "<oop>true</oop>" + "\n")
            if 'rights' in resource and len(resource['rights']) > 0:
                meta_file.write("\t" + "<aclrequest>" + "\n")
                for right in resource['rights']:
                    meta_file.write("\t\t" + '<right name="' + right + '" access="' +  str(resource['rights'][right]).lower() + '"></right>' + "\n")
                meta_file.write("\t" + "</aclrequest>" + "\n")
